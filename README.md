...
3. If the import fails, install once on that host:
...
5. Reload the BotRelay MCP server so `botrelay_mcp` loads `agent.env`. If the status stays Not connected, fully quit and reopen. Reload Window is not enough.

After that, every agent on that Grok account can use the vault. They share the one venv and `agent.env` on the VM.

Cursor on a Mac is a separate machine. The same API key and vault key are fine when both machines use the same agent vault. You are writing a second local `agent.env` on the Grok host.

The plugin starts `python -m botrelay_mcp` from `~/.venvs/botrelay` on the machine that is running the agent. The `botrelay_mcp` package loads that machine's `agent.env`. Marketplace `mcp.json` does not source the file.
...
A green BotRelay status in Cursor on your Mac does not mean the Grok virtual machine has `botrelay-mcp` or `~/.config/botrelay/agent.env`. Those paths are local to each machine. Configuring Cursor on a Mac does not create `agent.env` on the Grok VM. See [Grok Bot](#grok-bot).
